...
function browserDownloadAsync(
  text: string,
  name: string,
  contentType: string
): Promise<void> {
  pxt.BrowserUtils.browserDownloadBinText(text, name, {
...
export function browserDownloadDeployCoreAsync(
  ...
  if (userContext) {
    url = pxt.BrowserUtils.toDownloadDataUri(
      pxt.isOutputText() ? ts.pxtc.encodeBase64(out) : out,
      pxt.appTarget.compile.hexMimeType
    );
  } else if (!pxt.isOutputText()) {
    log("saving " + fn);
    url = pxt.BrowserUtils.browserDownloadBase64(out, fn, {
    ...
    log("saving " + fn);
    url = pxt.BrowserUtils.browserDownloadBinText(out, fn, {
  ...
  if (
    (!userContext && pxt.BrowserUtils.isBrowserDownloadInSameWindow()) ||
    (!resp.saveOnly && isDontShowDownloadDialogFlagSet())
  ) {
    return Promise.resolve().then(() => window.URL?.revokeObjectURL(url));
  } else {
...
function showUploadInstructionsAsync(
  ...
  const jsx =
    !userDownload &&
    pxt.commands.renderBrowserDownloadInstructions?.(saveonly, redeploy);
  const body = userDownload
    ? lf(
        "Click 'Download' to open the {0} app.",
        pxt.appTarget.appTheme.boardName
      )
    : !jsx &&
      lf(
  ...
  const timeout = pxt.BrowserUtils.isBrowserDownloadWithinUserContext()
    ? 0
    : 10000;
  return confirmAsync({
    header: userDownload
      ? lf("Download ready...")
      : lf("Download completed..."),
    body,
    jsx,
    hasCloseIcon: true,
    hideAgree: true,
...
export function nativeHostPostMessageFunction(): (
  msg: NativeHostMessage
) => void {
...
export async function hidDeployCoreAsync(
  resp: pxtc.CompileResult,
  d?: pxt.commands.DeployOptions
): Promise<void> {
  ...
  try {
    await pxt.Util.promiseTimeout(120000, deployCore());
  } catch (e) {
...
function applyExtensionResult() {
  ...
  if (res.renderBrowserDownloadInstructions) {
    ...
    pxt.commands.renderBrowserDownloadInstructions =
      res.renderBrowserDownloadInstructions;
  } else {
    // default
    pxt.commands.renderBrowserDownloadInstructions =
      dialogs.renderBrowserDownloadInstructions;
  }
  ...
  if (res.renderIncompatibleHardwareDialog) {
    ...
    pxt.commands.renderIncompatibleHardwareDialog =
      res.renderIncompatibleHardwareDialog;
  } else {
    // default
    pxt.commands.renderIncompatibleHardwareDialog =
      dialogs.renderIncompatibleHardwareDialog;
  }
  ...
  if (res.showProgramTooLargeErrorAsync) {
    ...
    pxt.commands.showProgramTooLargeErrorAsync =
      res.showProgramTooLargeErrorAsync;
  }
...
export async function initAsync() {
  ...
  if (
    (pxt.appTarget?.compile?.isNative || pxt.appTarget?.compile?.hasHex) &&
    ...
    if (
      ...
      if (
        !pxt.appTarget?.compile?.disableHIDBridge &&
  ...
  const forceBrowserDownload = /force(Hex)?(Browser)?Download/i.test(
  ...
  if (forceBrowserDownload || pxt.appTarget?.serial?.noDeploy) {
    ...
    pxt.commands.showUsbDeviceForgottenDialog =
      webusb.showDeviceForgottenDialog;
  } else if (hidbridge.shouldUse()) {
    log(`deploy: hid`);
    pxt.commands.deployCoreAsync = hidDeployCoreAsync;
  } else if (pxt.BrowserUtils.isLocalHost() && Cloud.localToken) {
    // local node.js
...
export async function maybeReconnectAsync(
  pairIfDeviceNotFound = false,
  skipIfConnected = false
): Promise<boolean> {
  log("[CLIENT]: starting reconnect");

  if (skipIfConnected && pxt.packetio.isConnected() && !disconnectingPacketIO)
    return true;
...
export function disconnectAsync(): Promise<void> {
  ...
  return pxt.packetio.disconnectAsync().then(() => {
...
async function requestPacketIOLockAsync() {
  if (hasLock) return;
  if (pendingPacketIOLockResolver)
    return Promise.reject("Already waiting for packet lock");
...
export async function handleServiceWorkerMessageAsync(
  message: pxt.ServiceWorkerMessage
) {
  if (
    message.action === "packet-io-lock-disconnect" &&
    !pendingPacketIOLockResolver &&
    lockRef === message.lock
  ) {
    ...
    message.action === "packet-io-lock-granted" &&
    message.lock === lockRef &&
    pendingPacketIOLockResolver
  ) {
    if (message.granted) {
    ...
    message.action === "packet-io-supported" &&
...
export function showUnsupportedHardwareMessageAsync(resp: pxtc.CompileResult) {
  ...
  if (unsupported.length) {
    ...
    const helpUrl =
      pxt.appTarget.appTheme.downloadDialogTheme?.incompatibleHardwareHelpURL;
    let cancelled = true;
    return core
      .confirmAsync({
