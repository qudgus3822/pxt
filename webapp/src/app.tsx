...
pxt.blocks.registerFieldEditor = (selector, proto, validator) =>
  pxtblockly.registerFieldEditor(selector, proto, validator);
  ...
  implements IProjectView
{
  ...
  constructor(props: IAppProps) {
    ...
    const isHighContrast =
      /hc=(\w+)/.test(window.location.href) ||
    ...
    this.state = {
      ...
      active:
        ...
        pxt.appTarget.appTheme.dontSuspendOnVisibility,
    ...
    if (!this.settings.editorFontSize)
      this.settings.editorFontSize = /mobile/i.test(navigator.userAgent)
        ? 15
        : 19;
    ...
    if (pxt.BrowserUtils.useOldTutorialLayout())
      this.hintManager.addHint(
        ProjectView.tutorialCardId,
        this.tutorialCardHintCallback.bind(this)
      );
  }
    ...
    window.addEventListener(
      ...
      (ev: MessageEvent) => {
        ...
        if (msg.type === "addextensions") {
          ...
          const handler =
            this.screenshotHandlers[this.screenshotHandlers.length - 1];
          if (handler) handler(scmsg);
          else {
            ...
            screenshot.saveAsync(this.state.header, pngString).then(() => {
          ...
          if (this.shareEditor)
            this.shareEditor.setThumbnailFrames(
              (msg as pxsim.SimulatorAutomaticThumbnailMessage).frames
            );
        } else if (msg.type === "multiplayer") {
          const multiplayerMessage = msg as pxsim.multiplayer.Message;
          if (
            multiplayerMessage.content === "Icon" &&
  ...
  private async addOrUpdateGithubExtensions(extensions: string[]) {
    ...
    const ghids = extensions
      ?.map((ext) => pxt.github.parseRepoId(ext))
      // make sure parsing succeeded
      .filter((ghid) => !!ghid)
      // remove anything banned
      .filter(
        (ghid) =>
          pxt.github.repoStatus(ghid, packageConfig) !==
          pxt.github.GitRepoStatus.Banned
      );

    // anything to add?
    if (!p || !ghids?.length) return;

    // need approval
    const ghidToBeApproved = ghids.filter(
      (ghid) =>
        pxt.github.repoStatus(ghid, packageConfig) !==
        pxt.github.GitRepoStatus.Approved
    );
      ...
      const ok = await core.confirmAsync({
        ...
        jsx: (
          <>
            <p>
              {lf(
                "Add or update these user-provided extensions to your project?"
              )}
            </p>
            <ul>
              {ghidToBeApproved.map((scr) => (
                <li key={scr.fullName}>
    ...
    try {
      ...
      for (const ghid of ghids) {
        ...
        const { config, version } = await pxt.github.downloadLatestPackageAsync(
          ghid
        );
  ...
  handleSetPresenceIcon = (
    ...
    if (!palette || !icon) {
      ...
      const iconPngDataUri = pxt.convertUint8BufferToPngUri(palette, icon);
      if (!iconPngDataUri) {
        // failed to parse icon, bail out
        return;
      }
      const asUrl = `url("${iconPngDataUri}")`;
      root.style.setProperty(slotCssVarName, asUrl);
    }
  };
  ...
  shouldShowHomeScreen() {
    ...
    const skipStartScreen =
      pxt.appTarget.appTheme.allowParentController ||
  ...
  async updateVisibilityAsync() {
    ...
    if (!active && !navigator?.serviceWorker?.controller) {
      if (this._deploying) {
        pxt.debug(
          `visibility: disconnect cancelled because deploy in progress`
        );
      } else cmds.disconnectAsync(); // turn off any kind of logging
    }
  ...
  saveSettings() {
    ...
    if (f && f.epkg.getTopHeader() && this.editor.hasHistory()) {
      this.pushFileHistory(
        f.epkg.getTopHeader().id,
        f.getName(),
        this.editor.getViewState()
      );
    }
  ...
  private pushFileHistory(id: string, name: string, pos: any) {
    ...
    sett.fileHistory = sett.fileHistory.filter(
      (e) => e.id != n.id || e.name != n.name
    );
    while (sett.fileHistory.length > 100) sett.fileHistory.pop();
  ...
  openProjectInLegacyEditor(majorVersion: number) {
    if (
      !this.editorFile ||
      !this.editorFile.epkg ||
      !this.editorFile.epkg.getTopHeader()
    )
    ...
    return workspace
      .copyProjectToLegacyEditor(header, majorVersion)
      .then((newHeader) => {
        ...
        if (
          appTheme &&
          appTheme.editorVersionPaths &&
          appTheme.editorVersionPaths[majorVersion]
        ) {
          const newPath = appTheme.editorVersionPaths[majorVersion];
          window.location.href = pxt.Util.pathJoin(
          ...
          window.location.href = pxt.Util.pathJoin(
            window.location.origin,
            "v" + header.targetVersion + "#editor"
          );
        }
      })
      .catch((e) => {
        core.warningNotification(
          lf("Importing to older editor version only supported in web browsers")
        );
      });
  }
  ...
  async componentDidUpdate(
    ...
    simulator.setState(
      this.state.header ? this.state.header.editor : "",
      this.state.tutorialOptions && !!this.state.tutorialOptions.tutorial
    );
  ...
  saveFileAsync(): Promise<void> {
    if (!this.editorFile || this.loadingExample) return Promise.resolve();
    return this.saveTypeScriptAsync().then(() => this.saveCurrentSourceAsync());
  }
  ...
  saveCurrentSourceAsync(): Promise<void> {
    ...
    if (txt != this.editorFile.content) simulator.setDirty();
  ...
  isBlocksActive(): boolean {
    return (
      !this.state.embedSimView &&
      this.editor == this.blocksEditor &&
      this.editorFile &&
      this.editorFile.name == pxt.MAIN_BLOCKS
    );
  }
  ...
  isJavaScriptActive(): boolean {
    return (
      ...
      this.editorFile &&
      this.editorFile.name == pxt.MAIN_TS
    );
  }
  ...
  isPythonActive(): boolean {
    return (
      ...
      this.editor == this.textEditor &&
      this.editorFile &&
      this.editorFile.name == pxt.MAIN_PY
    );
  }
  ...
  isAssetsActive(): boolean {
    return (
      !this.state.embedSimView &&
      this.editor == this.assetEditor &&
      this.editorFile &&
      this.editorFile.name == pxt.ASSETS_FILE
    );
  }
  ...
  private isAnyEditeableJavaScriptOrPackageActive(): boolean {
    return (
      this.editor == this.textEditor &&
      this.editorFile &&
      !this.editorFile.isReadonly() &&
      /(\.ts|pxt.json)$/.test(this.editorFile.name)
    );
  }
  ...
  openPython(giveFocusOnLoading = true) {
    ...
    if (this.isBlocksActive() || header.editor == pxt.BLOCKS_PROJECT_NAME) {
      ...
      if (!mainpy) this.updateFileAsync(pxt.MAIN_PY, "# ...", false);
      else this.setFile(mainpy);
    }
  }
  ...
  openBlocks() {
    ...
    if (
      this.isJavaScriptActive() ||
      (this.shouldTryDecompile && !this.state.embedSimView)
    )
      this.textEditor.openBlocks();
    // any other editeable .ts or pxt.json; or empty mainblocks
    else if (
      this.isAnyEditeableJavaScriptOrPackageActive() ||
      !mainBlocks.content
    ) {
  ...
  openSerial(isSim: boolean) {
    if (!pxt.appTarget.serial || !pxt.appTarget.serial.useEditor) return; // not supported in this editor
  ...
  openPythonAsync(): Promise<void> {
    ...
    if (pySrcFile) {
      ...
      return this.textEditor
        .convertPythonToTypeScriptAsync()
        .then((pyAsTsSrc) => {
          if (pyAsTsSrc == tsSrc) {
  ...
  private convertTypeScriptToPythonAsync() {
    ...
    if (cached) {
      convertPromise = this.saveVirtualMainFileAsync(
        pxt.PYTHON_PROJECT_NAME,
        cached,
        true
      );
    } else {
      convertPromise = this.saveTypeScriptAsync(false)
        ...
        .then((cres) => {
          if (cres && cres.success) {
            ...
            return this.saveVirtualMainFileAsync(
              pxt.PYTHON_PROJECT_NAME,
              mainpy,
              true
            );
    ...
    const convertPromiseOrErrorDialog = convertPromise.then(
      () => {},
      (e) => {
    ...
    core.showLoadingAsync(
      "switchtopython",
      lf("switching to Python..."),
      convertPromise
    );
  ...
  private maybeShowPackageErrors(force = false) {
    ...
    if (!this.state.suppressPackageWarning || force) {
      ...
      if (badPackages.length) {
        ...
        if (
          workspace.isBrowserWorkspace() &&
          currentVersion.major !== headerVersion.major
        ) {
          openHandler = () => {
        ...
        dialogs
          .showPackageErrorDialogAsync(
            badPackages,
            compiler.updatePackagesAsync,
            openHandler
          )
          .then((projectOpen) => {
            if (projectOpen) {
  ...
  private typecheckDebouncer = new pxtc.Util.AdaptiveDebouncer(() => {
    ...
    compiler.typecheckAsync().then((resp) => {
      const end = Util.now();
      ...
      if (
        ...
        this._slowTypeCheck++ > 1
      ) {
        pxt.tickEvent("light.typecheck");
        pxt.options.light = true;
      }
      pxt.tickEvent("typecheck.complete", {
      ...
      if (this.state.autoRun) {
        ...
        if (output && !output.numDiagnosticsOverride && this.state.autoRun) {
    ...
    let changeHandler = () => {
      if (this.editorFile) {
        if (this.editorFile.inSyncWithEditor)
          pxt.tickEvent(
            "activity.edit",
            { editor: this.editor.getId().replace(/Editor$/, "") },
      ...
      if (
        ...
        pxt.appTarget.simulator &&
        pxt.appTarget.simulator.stopOnChange
      )
        this.stopSimulator();

      if (!this.editor.isIncomplete()) {
        if (this.editor == this.textEditor) this.typecheckDebouncer.poke();
    ...
    await simulator.initAsync(document.getElementById("boardview"), {
      ...
      highlightStatement: (stmt, brk) => {
        if (
          ...
          !simulator.driver.areBreakpointsSet() &&
          brk &&
          !brk.exceptionMessage
        ) {
  ...
  handleCriticalError(error: any, info?: any) {
    try {
      ...
      // Log critical error
      pxt.tickEvent("pxt.criticalerror", {
        error: error || "",
        info: info || "",
      });
      // Reload the page in 2 seconds
      const lastCriticalError =
        pxt.storage.getLocal("lastcriticalerror") &&
        Date.parse(pxt.storage.getLocal("lastcriticalerror"));
      // don't refresh if we refreshed in the last minute
      if (
        !lastCriticalError ||
        isNaN(lastCriticalError) ||
        Date.now() - lastCriticalError > 60 * 1000
      ) {
  ...
  private updateEditorFileAsync(editorOverride: srceditor.Editor = null) {
    if (
      ...
      if (
        ...
        this.editorFile == this.state.editorPosition.file
      ) {
    ...
    return core
      ...
      .then(() => {
        ...
        if (!this.state.currFile.virtual && !this.state.debugging) {
          // switching to serial should not reset the sim
          this.stopSimulator();
      ...
      .then(() => {
        ...
        return previousEditor
          ? previousEditor.unloadFileAsync()
          : Promise.resolve();
      })
      .then(() => {
      ...
      .then(() => {
        ...
        if (this.state.editorPosition) {
          ...
          const e = this.settings.fileHistory.find(
            (e) =>
              e.id == this.state.header.id &&
              e.name == this.editorFile.getName()
          );
          if (e) this.editor.setViewState(e.pos);
        }
        ...
        if (this.state.showBlocks && this.editor == this.textEditor)
          this.textEditor.openBlocks();
      })
      .finally(() => {
      ...
      .then(() => {
        ...
        if (!this.state.currFile.virtual && simRunning && !this.state.autoRun)
          this.startSimulator();
        else this.typecheck(); // trigger at least 1 auto-run in non-code editors
      });
  }
  ...
  setFile(fn: pkg.File, line?: number) {
    ...
    if (header) {
      ...
      if (pkgId === "this") {
        ...
        if (isCodeFile) pkg.mainPkg.setPreferredEditor(header.editor);
      }
    }
  ...
  navigateToError(diag: pxtc.KsDiagnostic) {
    ...
    if (!/^(pxt_modules|built)\//.test(fn)) fn = "this/" + fn;
  ...
  setTutorialInstructionsExpanded(value: boolean): void {
    ...
    this.setState({ tutorialOptions: tutorialOptions }, () => {
      if (this.editor == this.blocksEditor) this.blocksEditor.hideFlyout();
      else if (this.editor == this.textEditor) this.textEditor.hideFlyout();
  ...
  setTutorialStep(step: number) {
    ...
    if (step > -1) {
      ...
      const isCompleted =
        tutorialOptions.tutorialStepInfo[step].tutorialCompleted;
  ...
  protected postTutorialProgress() {
    const currentStep =
      this.state.tutorialOptions?.tutorialStep ||
      this.state.header?.tutorialCompleted?.steps ||
      0;
    const totalSteps =
      this.state.tutorialOptions?.tutorialStepInfo?.length ||
      this.state.header?.tutorialCompleted?.steps ||
      0;
    const tutorialId =
      this.state.tutorialOptions?.tutorial ||
      this.state.header?.tutorialCompleted.id;
  ...
  tryCheckTargetVersionAsync(targetVersion: string): Promise<void> | undefined {
    ...
    const legacyProject =
      pxt.semver.majorCmp(htv, pxt.appTarget.versions.target) < 0;
    if (legacyProject)
    ...
    if (pxt.semver.majorCmp(htv, pxt.appTarget.versions.target) > 0) {
      ...
      if (
        ...
        pxt.appTarget.appTheme.homeUrl
      )
        buttons.push({
          label: lf("Get latest"),
  ...
  async loadHeaderAsync(
    h: pxt.workspace.Header,
    ...
    if (tryCloudSync) {
      ...
      await Promise.race([
        ...
        cloud.syncAsync({ hdrs: [h], direction: "down" }).then((changes) => {
          if (changes.length) {
            ...
            if (timedOut) {
              ...
              pxt.tickEvent(`identity.syncOnProjectOpen.timedout`, {
                elapsedSec: elapsed,
              });
              ...
              pxt.tickEvent(`identity.syncOnProjectOpen.syncSuccess`, {
                elapsedSec: elapsed,
              });
  ...
  private async internalLoadHeaderAsync(
    h: pxt.workspace.Header,
    editorState?: pxt.editor.EditorState
  ): Promise<void> {
    ...
    try {
      ...
      const e =
        h.editor != pxt.BLOCKS_PROJECT_NAME &&
        this.settings.fileHistory.filter((e) => e.id == h.id)[0];
      if (e) {
        ...
        if (lastEdited) {
          ...
          file =
            main.lookupFile("this/" + file.getFileNameWithExtension("ts")) ||
            file;
        } else if (pkg.getExtensionOfFileName(e.name) === "py") {
          file =
            main.lookupFile("this/" + file.getFileNameWithExtension("py")) ||
            file;
        }
      }
      ...
      if (!e && h.editor && file.getVirtualFileName(h.editor)) {
        file =
          main.lookupFile("this/" + file.getVirtualFileName(h.editor)) || file;
      }
      ...
      if (pxteditor.isBlocks(file) && !file.content) {
        if (!file.content) {
          ...
          file =
            main.lookupFile(
              "this/" + file.getVirtualFileName(pxt.JAVASCRIPT_PROJECT_NAME)
            ) || file;
        }
      }
      ...
      /* await */ pkg.mainPkg.getCompileOptionsAsync().catch((e) => {
      ...
      if (documentation) {
        ...
        if (
          !h.githubId &&
          readmeContent &&
          !/#{2,}\s+@autoOpen\s+false\s*/i.test(readmeContent)
        ) {
  ...
  private async loadTutorialFiltersAsync(): Promise<void> {
    ...
    const skipTutorialInfoCache = /notutorialinfocache=1/i.test(
    ...
    try {
      ...
      const tutorialBlocks = await tutorial.getUsedBlocksAsync(
        t.tutorialCode,
        t.tutorial,
        t.language,
        skipTutorialInfoCache
      );
      let editorState: pxt.editor.EditorState = {};

      if (
        tutorialBlocks?.usedBlocks &&
        Object.keys(tutorialBlocks.usedBlocks).length > 0
      ) {
        editorState.filters = {
        ...
        editorState.hasCategories = !(
          header.tutorial.metadata && header.tutorial.metadata.flyoutOnly
        );
      }
      ...
      core.warningNotification(
        lf("Could not filter tutorial blocks, displaying full toolbox.")
      );
    } finally {
  ...
  private async loadTutorialBlockConfigsAsync(): Promise<void> {
    const resolveBlockConfigAsync = async (
      ...
      try {
        ...
        const decomp = await compiler.decompileBlocksSnippetAsync(
          blockConfig.md,
          undefined,
          {
            snippetMode: true,
        ...
        pxt.error(
          `Failed to resolve blockconfig for tutorial: ${header.tutorial.tutorialName}, ${e.message}. md:${blockConfig.md}`
        );
      }
      ...
      for (const block of blocks) {
        try {
          ...
          switch (blockType) {
            ...
            case "variables_change": {
              ...
              const value = Array.from(block.children).find(
                (child) => child.tagName === "value"
              );
              const rhs = Array.from(value.children).find(
          ...
          entry.xml = Blockly.Xml.domToText(block)
            .replace(/^<xml[^>]*>\n*/i, "")
            .replace(/\n*<\/xml>\n*$/i, "");
          ...
          pxt.error(
            `Failed to resolve blockconfig for tutorial: ${header.tutorial.tutorialName}. ${e.message}`
          );
        }
      }
    };
    ...
    stepInfo
      ...
      .forEach((blockConfig) =>
        tasks.push(resolveBlockConfigAsync(blockConfig))
      );
  ...
  private async loadTutorialTemplateCodeAsync(): Promise<void> {
    ...
    if (!header || !header.tutorial) {
      ...
      !header.tutorial.templateCode ||
      header.tutorial.templateLoaded
    ) {
      if (header.tutorial.mergeCarryoverCode && header.tutorial.mergeHeaderId) {
        pxt.warn(
          lf(
            'Refusing to carry code between tutorials because the loaded tutorial "{0}" does not contain a template code block.',
    ...
    if (
      preferredEditor &&
      filenameForEditor(preferredEditor) === pxt.ASSETS_FILE
    ) {
    ...
    if (
      projectname === pxt.PYTHON_PROJECT_NAME &&
      header.tutorial.templateLanguage === "python"
    ) {
    ...
    if (header.tutorial.mergeHeaderId) {
      ...
      if (previousHeader) {
        ...
        newText = mergeProjectCode(
          previousText,
          currentText,
          !!header.tutorial.mergeCarryoverCode
        );
      }
    }
    ...
    try {
      parsedTilemap = newText[pxt.TILEMAP_JRES]
        ? JSON.parse(newText[pxt.TILEMAP_JRES])
        : null;
      parsedImage = newText[pxt.IMAGES_JRES]
        ? JSON.parse(newText[pxt.IMAGES_JRES])
        : null;
    } catch (e) {
      return Promise.reject(e);
    }
  ...
  private async loadTutorialJresCodeAsync(): Promise<void> {
    ...
    if (
      !header ||
      !header.tutorial ||
      !(header.tutorial.jres || header.tutorial.assetFiles)
    ) {
      return;
    }

    const tilemapJRes =
      header.tutorial.jres || header.tutorial.assetFiles?.[pxt.TILEMAP_JRES];
  ...
  async resetTutorialTemplateCode(keepAssets: boolean): Promise<void> {
    ...
    if (keepAssets) {
      ...
      let currentText = await workspace.getTextAsync(header.id);
      const imageJres = appendTemporaryAssets(
        currentText[pxt.MAIN_BLOCKS],
        currentText[pxt.IMAGES_JRES]
      );
  ...
  removeProject() {
    ...
    core.confirmDelete(pkg.mainEditorPkg().header.name, () => {
      let curr = pkg.mainEditorPkg().header;
      curr.isDeleted = true;
      return workspace.forceSaveAsync(curr, {}).then(() => this.openHome());
    });
  }
  ...
  hexFileImporters: pxt.editor.IHexFileImporter[] = [
    {
      id: "default",
      canImport: (data) =>
        data.meta.cloudId == "ks/" + pxt.appTarget.id ||
        data.meta.cloudId == pxt.CLOUD_ID + pxt.appTarget.id || // match on targetid
      ...
      importAsync: (project, data) => {
        let h: pxt.workspace.InstallHeader = {
          ...
          targetVersion: data.meta.targetVersions
            ? data.meta.targetVersions.target
            : undefined,
  ...
  isAssetFile(filename: string): boolean {
    let exts = pxt.appTarget.runtime
      ? pxt.appTarget.runtime.assetExtensions
      : null;
    if (exts) {
  ...
  importProjectCoreAsync(
    buf: Uint8Array,
    options?: pxt.editor.ImportFileOptions
  ) {
    return (
      buf[0] == "{".charCodeAt(0)
      ...
      .then((contents) => {
        ...
        if (
          parsedContents.target &&
          parsedContents.target == pxt.appTarget.id
        ) {
      ...
      .catch((e) => {
        core.warningNotification(
          lf("Sorry, we could not import this project or block snippet.")
        );
        this.openHome();
      });
  }
  ...
  importHexFile(file: File, options?: pxt.editor.ImportFileOptions) {
    if (!file) return;
    pxt.cpp
      .unpackSourceFromHexFileAsync(file)
      .then((data) => this.importHex(data, options))
      .catch((e) => {
        pxt.reportException(e);
        core.warningNotification(
          lf("Sorry, we could not recognize this file.")
        );
      });
  }
  ...
  importBlocksFiles(file: File, options?: pxt.editor.ImportFileOptions) {
    if (!file) return;
    ts.pxtc.Util.fileReadAsTextAsync(file).then((contents) => {
      this.newProject({
  ...
  importProjectFile(file: File, options?: pxt.editor.ImportFileOptions) {
    if (!file) return;
    ts.pxtc.Util.fileReadAsBufferAsync(file).then((buf) =>
      this.importProjectCoreAsync(buf, options)
    );
  }
  ...
  importPNGFile(file: File, options?: pxt.editor.ImportFileOptions) {
    ...
    ts.pxtc.Util.fileReadAsBufferAsync(file)
      .then((buf) =>
        pxt.Util.decodeBlobAsync(
          "data:image/png;base64," + btoa(pxt.Util.uint8ArrayToString(buf))
        )
      )
  ...
  async importZipFileAsync(file: File, options?: pxt.editor.ImportFileOptions) {
    ...
    const zippedFiles = (await reader.getEntries()).filter((zipped: any) =>
      this.isProjectFile(zipped.filename)
    );
    let progress = {
      done: 0,
    };
    ...
    if (zippedFiles.length === 0) {
      core.warningNotification(
        lf("No projects available to import found inside zip file.")
      );
      return;
    }
    ...
    const confirmed = await core.confirmAsync({
      ...
      body: lf(
        "Do you want to import all projects in this zip file? This will import {0} projects.",
        zippedFiles.length
      ),
      agreeLbl: lf("Okay"),
      hasCloseIcon: true,
    ...
    core.dialogAsync({
      ...
      jsxd: () => (
        <dialogs.ProgressBar
          percentage={100 * (progress.done / zippedFiles.length)}
        />
      ),
    ...
    for (const zipped of zippedFiles) {
      try {
        const buf: Uint8Array = await zipped.getData(
          new zip.Uint8ArrayWriter()
        );

        const data = JSON.parse(
          await pxt.lzmaDecompressAsync(buf)
        ) as pxt.cpp.HexFile;
  ...
  importHex(data: pxt.cpp.HexFile, options?: pxt.editor.ImportFileOptions) {
    ...
    if (
      this.hexFileImporters.some(
        (fi) => fi.id == "default" && fi.canImport(data)
      )
    ) {
      const checkAsync = this.tryCheckTargetVersionAsync(
        data.meta.targetVersions && data.meta.targetVersions.target
      );
      if (checkAsync) {
        checkAsync.then(() => {
    ...
    if (options && options.extension) {
      ...
      if (files) {
        ...
        if (mpkg) {
          ...
          // save file
          mpkg
            .setContentAsync(fn, data.source)
            .then(() =>
              mpkg.updateConfigAsync((cfg) => {
    ...
    const importer = this.hexFileImporters.filter((fi) =>
      fi.canImport(data)
    )[0];
    if (importer) {
      ...
      pxteditor.initEditorExtensionsAsync().then(() =>
        importer.importAsync(this, data).then(
          () => core.hideLoading("importhex"),
          (e) => {
            pxt.reportException(e, { importer: importer.id });
            core.hideLoading("importhex");
            core.errorNotification(
              lf("Oops, something went wrong when importing your project")
            );
            if (options && options.openHomeIfFailed) this.openHome();
          }
        )
  ...
  protected async installAndLoadProjectAsync(
    project: pxt.workspace.Project,
    editorState?: pxt.editor.EditorState
  ) {
  ...
  importTutorialAsync(md: string) {
    try {
      const { options, editor } = pxt.tutorial.getTutorialOptions(
        md,
        "untitled",
        "untitled",
        "",
  ...
  initDragAndDrop() {
    draganddrop.setupDragAndDrop(
      document.body,
      (file) =>
        (file.size < 1000000 && this.isHexFile(file.name)) ||
        this.isBlocksFile(file.name) ||
        this.isZipFile(file.name),
      (files) => {
        if (files) {
  ...
  importFile(file: File, options?: pxt.editor.ImportFileOptions) {
    ...
    if (this.isHexFile(file.name)) {
      ...
      const importer = this.resourceImporters.filter((fi) =>
        fi.canImport(file)
      )[0];
      if (importer) {
  ...
  importProjectFromFileAsync(
    buf: Uint8Array,
    options?: pxt.editor.ImportFileOptions
  ): Promise<void> {
    return pxt
      .lzmaDecompressAsync(buf)
      .then((project) => {
  ...
  syncPreferredEditor() {
    ...
    if (pe) pkg.mainPkg.setPreferredEditor(pe);
  }
  ...
  exportAsync(): Promise<string> {
    ...
    return this.exportProjectToFileAsync().then((buf) => {
  ...
  downloadScreenshotAsync(): Promise<void> {
    if (pxt.appTarget.compile.saveAsPNG)
      ...
      return this.requestScreenshotAsync().then((img) =>
        pxt.BrowserUtils.browserDownloadDataUri(img, pkg.genFileName(".png"))
      );
  }

  pushScreenshotHandler = (
    handler: (msg: pxt.editor.ScreenshotData) => void
  ): void => {
  ...
  requestScreenshotAsync(): Promise<string> {
    if (this.requestScreenshotPromise) return this.requestScreenshotPromise;
    ...
    return Util.promiseTimeout(
      1000,
      (this.requestScreenshotPromise = new Promise<string>(
        (resolve, reject) => {
          this.pushScreenshotHandler((msg) =>
            resolve(pxt.BrowserUtils.imageDataToPNG(msg.data, 3))
          );
        }
      ))
  ...
  private encodeProjectAsPNGAsync(
    sc: string,
    showDialog: boolean
  ): Promise<void> {
    return this.exportProjectToFileAsync()
      .then((blob) =>
        screenshot.encodeBlobAsync(this.state.header.name, sc, blob)
      )
      .then((img) => {
              ...
              <div className="item">
                <div className="ui small image">
                  <a download={fn} className="ui link" href={img}>
                    <img
                      src={img}
                      alt={lf("Project cartridge")}
                      title={lf("Click to download again")}
                    />
                  </a>
                </div>
  ...
  private saveProjectAsPNGAsync(showDialog: boolean): Promise<void> {
    return this.requestScreenshotAsync().then((img) =>
      this.encodeProjectAsPNGAsync(img, showDialog)
    );
  }
  ...
  saveProjectToFileAsync(): Promise<void> {
    ...
    if (saveAsBlocks()) {
      pxt.BrowserUtils.browserDownloadText(
        mpkg.readFile(pxt.MAIN_BLOCKS),
        pkg.genFileName(".blocks"),
    ...
    if (pxt.appTarget.compile.saveAsPNG)
      ...
      return this.exportProjectToFileAsync().then((buf: Uint8Array) => {
        const fn = pkg.genFileName(".mkcd");
        pxt.BrowserUtils.browserDownloadUInt8Array(buf, fn, {
  ...
  openHome() {
    const hasHome =
      !pxt.shell.isControllerMode() && !pxt.appTarget.appTheme.lockedEditor;
    if (!hasHome) return;
  ...
  getPreferredEditor(): string {
    if (this.editor == this.blocksEditor) return pxt.BLOCKS_PROJECT_NAME;
  ...
  async createProjectAsync(options: ProjectCreationOptions): Promise<void> {
    ...
    if (options.languageRestriction) {
      ...
      if (
        options.languageRestriction ===
        pxt.editor.LanguageRestriction.JavaScriptOnly
      ) {
        filesToDrop = /\.(py|blocks)$/;
        cfg.preferredEditor = pxt.JAVASCRIPT_PROJECT_NAME;
      } else if (
        options.languageRestriction ===
        pxt.editor.LanguageRestriction.PythonOnly
      ) {
    ...
    if (options.tutorial && options.tutorial.metadata) {
      if (options.tutorial.metadata.codeStart) {
        ...
        ] = `control._onCodeStart('${pxt.U.htmlEscape(
          options.tutorial.metadata.codeStart
        )}')`;
        cfg.files.splice(
          cfg.files.indexOf(pxt.MAIN_TS),
          0,
          pxt.TUTORIAL_CODE_START
        );
      }
      if (options.tutorial.metadata.codeStop) {
        ...
        ] = `control._onCodeStop('${pxt.U.htmlEscape(
          options.tutorial.metadata.codeStop
        )}')`;
  ...
  private autoChooseBoardAsync(features?: string[]): Promise<void> {
    if (
      pxt.appTarget.appTheme.chooseBoardOnNewProject &&
      pxt.appTarget.simulator &&
  ...
  importExampleAsync(options: pxt.editor.ExampleImportOptions): Promise<void> {
    ...
    return this.loadActivityFromMarkdownAsync(
      path,
      name.toLowerCase(),
      preferredEditor
    )
      .then((r) => {
        const {
          ...
          md,
          features,
          autoChooseBoard: autoChooseBoardMeta,
        } = r || {};
        ...
        if (loadBlocks && preferredEditor == pxt.BLOCKS_PROJECT_NAME) {
          return this.createProjectAsync(opts).then(() => {
            return this.loadBlocklyAsync()
              .then(compiler.getBlocksAsync)
              .then((blocksInfo) =>
                compiler.decompileAsync(pxt.MAIN_TS, blocksInfo)
              )
              .then((resp) => {
              ...
              .then(
                () => autoChooseBoard && this.autoChooseBoardAsync(features)
              );
          });
        } else {
          if (!loadBlocks) {
          ...
          return this.createProjectAsync(opts).then(() => {
            ...
            if (
              ...
              example.snippetType !== "python"
            ) {
              return compiler
                .getApisInfoAsync() // ensure compiled
                .then(() => compiler.pyDecompileAsync(pxt.MAIN_TS))
                .then((resp) => {
                  pxt.debug(`example decompilation: ${resp.success}`);
                  if (resp.success) {
      ...
      .catch((e) => {
        core.warningNotification(
          lf(
            "Please check your internet connection and check the example is valid."
          )
        );
        pxt.reportException(e);
        return Promise.reject(e);
  ...
  private saveVirtualMainFileAsync(
    prj: string,
    src: string,
    open: boolean
  ): Promise<void> {
  ...
  private saveVirtualFileAsyncInternal(
    prj: string,
    src: string,
    open: boolean,
    fileName: string
  ...
  saveTypeScriptAsync(open = false): Promise<void> {
    ...
    promise = promise
      .then(() => {
        if (open) {
          ...
          const cached = mainPkg.getCachedTranspile(
            fromLanguage,
            fromText,
            "ts"
          );
          ...
          return this.editor.saveToTypeScriptAsync(true).then((src) => {
            if (src !== undefined) {
      ...
      .then((src) => {
        ...
        if (
          src === undefined ||
          (this.editorFile &&
            this.editorFile.name ==
              this.editorFile.getVirtualFileName(pxt.JAVASCRIPT_PROJECT_NAME))
        )
          return Promise.resolve();
        if (this.editorFile.getVirtualFileName(pxt.JAVASCRIPT_PROJECT_NAME))
          return this.saveVirtualFileAsync(
            pxt.JAVASCRIPT_PROJECT_NAME,
            src,
            open
          );
    ...
    if (open) {
      return core.showLoadingAsync(
        "switchtojs",
        lf("switching to JavaScript..."),
        promise,
        0
      );
    } else {
  ...
  resetWorkspace() {
    this.reload = true;
    return workspace.resetAsync().then(
      () => this.reloadEditor(),
      () => this.reloadEditor()
    );
  }
  ...
  saveAndCompile() {
    ...
    return (
      this.state.projectName !== lf("Untitled")
      ...
      return this.saveProjectNameAsync()
        ...
        .then(() => {
          if (
            !pxt.appTarget.compile.hasHex ||
            pxt.appTarget.compile.useMkcd ||
            pxt.appTarget.compile.saveAsPNG ||
            saveAsBlocks() ||
            saveTutorialTemplate()
          ) {
            this.saveProjectToFileAsync().finally(() => {
              this.setState({ isSaving: false });
            });
          } else {
            this.compile(true);
  ...
  checkForHwVariant() {
    if (pxt.hwVariant || !pxt.hasHwVariants()) return false; // already set
          ...
          if (wr.familyID) {
            for (let v of variants) {
              ...
              if (v.compileServiceVariant) {
                ...
                if (c2.compile) pxt.U.jsonCopyFrom(compile, c2.compile);
              }
              if (parseInt(compile.uf2Family) === wr.familyID) {
                ...
                this.reloadHeaderAsync().then(() => this.compile());
                return;
              }
            }
          }
  ...
  async compileOnlyPostMessage(): Promise<void> {
    await this.compile(false, true);
  }

  async compile(saveOnly = false, onlyPostMessage = false): Promise<void> {
    pxt.tickEvent("compile", { editor: this.getPreferredEditor() });
    pxt.debug("compiling...");

    if (this.checkForHwVariant()) return;
    ...
    if (
      !pxt.appTarget.compile.useModulator &&
    ...
    try {
      ...
      debugger
      try {
        const resp = await compiler.compileAsync({
          native: true,
          forceEmit: true,
        });
        if (onlyPostMessage) {
        ...
        if (!saveOnly) {
          const shouldContinue = await cmds.showUnsupportedHardwareMessageAsync(
            resp
          );
          if (!shouldContinue) {
        ...
        if (!resp.outfiles[fn]) {
          ...
          const noHexFileDiagnostic =
            resp.diagnostics.find((diag) => diag.code === 9043) ||
            resp.diagnostics.length == 1
              ? resp.diagnostics[0]
              : undefined;

          if (
            noHexFileDiagnostic?.code === 9283 /*program too large*/ &&
            pxt.commands.showProgramTooLargeErrorAsync
          ) {
            pxt.tickEvent("compile.programTooLargeDialog");
            const res = await pxt.commands.showProgramTooLargeErrorAsync(
              attemptedVariants,
              core.confirmAsync,
              saveOnly
            );
          ...
          if (noHexFileDiagnostic) {
            ...
            core.warningNotification(
              lf("Compilation failed, please check your code for errors.")
            );
          }
          return;
        }
        ...
        try {
          ...
          if (e.notifyUser) {
            ...
            const errorText = pxt.appTarget.appTheme.useUploadMessage
              ? lf("Upload failed, please try again.")
        ...
        pxt.reportException(e);
        core.errorNotification(
          lf("Compilation failed, please contact support.")
        );
        if (userContextWindow)
          try {
            userContextWindow.close();
  ...
  async setLanguageRestrictionAsync(
  ...
  toggleTrace(intervalSpeed?: number) {
    ...
    if (tracing) {
      ...
      simulator.setTraceInterval(
        intervalSpeed || simulator.SLOW_TRACE_INTERVAL
      );
    }
  ...
  toggleSimulatorCollapse() {
    ...
    pxt.tickEvent(
      "simulator.toggleCollapse",
      { view: "computer", collapsedTo: "" + !state.collapseEditorTools },
      { interactiveConsent: true }
    );
    if (
      state.simState == SimState.Stopped &&
      state.collapseEditorTools &&
      !pxt.appTarget.simulator.headless
    ) {
      this.startStopSimulator();
    }
  ...
  printCode() {
    ...
    const mode = theEditor.isBlocksActive()
      ? "blocks"
      : theEditor.isPythonActive()
      ? "python"
      : "typescript";
  ...
  restartSimulator() {
    ...
    if (this.state.simState == SimState.Stopped || this.debugOptionsChanged()) {
      this.startSimulator();
    } else {
  ...
  debugOptionsChanged() {
    ...
    return (
      !!debugging != simulator.driver.isDebug() ||
      !!tracing != simulator.driver.isTracing()
    );
  }
  ...
  stopSimulator(unload?: boolean, opts?: pxt.editor.SimulatorStartOptions) {
    ...
    if (
      ...
      (unload && simulator.driver?.state !== pxsim.SimulatorState.Unloaded)
    ) {
  ...
  runSimulator(opts: compiler.CompileOptions = {}): Promise<void> {
    const emptyRun =
      ...
      !!pxt.appTarget.simulator.emptyRunCode &&
      !this.isBlocksEditor();
    if (this.firstRun && pxt.BrowserUtils.isSafari())
      this.setMute(pxt.editor.MuteState.Disabled);

    pxt.debug(
      `sim: start run (autorun ${this.state.autoRun}, first ${this.firstRun})`
    );
    this.firstRun = false;
    ...
    return (() => {
      const editorId = this.editor
        ? this.editor.getId().replace(/Editor$/, "")
        : "unknown";
      if (opts.background) {
        pxt.tickEvent("activity.autorun", { editor: editorId });
        if (pxt.storage.getLocal("noAutoRun")) return Promise.resolve();
      } else pxt.tickEvent(opts.debug ? "debug" : "run", { editor: editorId });

      if (!opts.background) this.editor.beforeCompile();
      if (this.state.tracing && this.state.debugging) opts.trace = true;

      if (opts.debug) {
        opts.debugExtensionCode =
          pxt.appTarget.appTheme.debugExtensionCode && !this.isBlocksActive();
      }
      ...
      const autoRun =
        this.autoRunOnStart() &&
        this.isBlocksEditor() &&
        (this.state.autoRun || !!opts.clickTrigger);

      const state = this.editor.snapshotState();
      return this.setStateAsync({
        simState: SimState.Starting,
        autoRun: autoRun,
      })
        .then(() =>
          emptyRun
            ? Promise.resolve(compiler.emptyCompileResult())
        ...
        .then((resp) => {
          ...
          if (resp.outfiles[pxtc.BINARY_JS]) {
            if (!cancellationToken.isCancelled()) {
              ...
              this.blocksEditor.setBreakpointsMap(
                resp.breakpoints,
                resp.procCallLocations
              );
              this.textEditor.setBreakpointsMap(
                resp.breakpoints,
                resp.procCallLocations
              );
              if (!cancellationToken.isCancelled()) {
  ...
  openNewTab(hd: pxt.workspace.Header, dependent: boolean) {
    if (!hd || pxt.BrowserUtils.isElectron() || pxt.BrowserUtils.isIOS())
      return;
    ...
    if (dependent)
      url =
        url +
        (url.indexOf("?") > -1 ? "&" : "?") +
        "nestededitorsim=1&lockededitor=1";
    url += "#header:" + hd.id;
  ...
  createGitHubRepositoryAsync(): Promise<void> {
    ...
    return cloudsync
      .githubProvider(true)
      .createRepositoryAsync(projectName, header)
  ...
  hwDebug() {
    ...
    return start.then(() => {
      ...
      pxt.HWDBG.postMessage = (msg) =>
        simulator.driver.handleHwDebuggerMsg(msg);
      return Promise.all([
        compiler.compileAsync({ debug: true, native: true }),
        pxt.packetio.initAsync(),
      ]).then((vals) =>
        pxt.HWDBG.startDebugAsync(vals[0], vals[1] as pxt.HF2.Wrapper)
      );
    });
  }
  ...
  renderPythonAsync(
    req: pxt.editor.EditorMessageRenderPythonRequest
  ): Promise<pxt.editor.EditorMessageRenderPythonResponse> {
    return compiler.decompilePythonSnippetAsync(req.ts).then((resp) => {
      return { python: resp };
    });
  }
  ...
  blocksScreenshotAsync(
    pixelDensity?: number,
    encodeBlocks?: boolean
  ): Promise<string> {
    if (
      pxtblockly.screenshotEnabled() &&
      this.blocksEditor &&
      this.blocksEditor.isReady &&
      this.blocksEditor.editor
    )
      return pxtblockly.screenshotAsync(
        this.blocksEditor.editor,
        pixelDensity,
        encodeBlocks
      );
  ...
  renderBlocksAsync(
    req: pxt.editor.EditorMessageRenderBlocksRequest
  ): Promise<pxt.editor.EditorMessageRenderBlocksResponse> {
    return compiler
      .getBlocksAsync()
      .then((blocksInfo) =>
        compiler.decompileBlocksSnippetAsync(req.ts, blocksInfo, req)
      )
      .then((resp) => {
        const svg = pxtblockly.render(resp.outfiles[pxt.MAIN_BLOCKS], {
          snippetMode: req.snippetMode || false,
          layout:
            req.layout !== undefined
              ? req.layout
              : pxt.editor.BlockLayout.Align,
          splitSvg: false,
        }) as SVGSVGElement;
        ...
        return {
          svg: svg,
          xml: pxtblockly.blocklyToSvgAsync(
            svg,
            viewBox[0],
            viewBox[1],
            viewBox[2],
  ...
  renderXmlInner(
    ...
    const wrappedXml = xml.trim().toLocaleLowerCase().startsWith("<xml")
      ? xml
    ...
    return {
      svg: svg,
      resultXml: pxtblockly.blocklyToSvgAsync(
        svg,
        viewBox[0],
        viewBox[1],
        viewBox[2],
  ...
  renderXml(
    req: pxt.editor.EditorMessageRenderXmlRequest
  ): pxt.editor.EditorMessageRenderXmlResponse {
    return this.renderXmlInner(req.xml, req.snippetMode, req.layout);
  }

  async renderByBlockIdAsync(
    ...
    const symbolInfo: pxtc.SymbolInfo =
      blocksInfo.apis.byQName[blockInfo.qName];
    const compileInfo: pxt.blocks.BlockCompileInfo =
      pxt.blocks.compileInfo(symbolInfo);
    const xml = pxtblockly.createToolboxBlock(
      blocksInfo,
      symbolInfo,
      compileInfo,
      false,
  ...
  getToolboxCategories(
    advanced?: boolean
  ): pxt.editor.EditorMessageGetToolboxCategoriesResponse {
    if (!this.isBlocksActive()) {
    ...
    const categories: pxt.editor.ToolboxCategoryDefinition[] =
      categoriesInternal.map((c) => {
        return {
          ...c,
  ...
  getBlocksWithId(blockId: string): BlockDefinition[] {
    ...
    for (const advanced of [false, true]) {
      for (const category of this.blocksEditor.getToolboxCategories(advanced)) {
        for (const match of category.blocks?.filter(
  ...
  getBlockAsText(blockId: string): pxt.editor.BlockAsText | undefined {
    ...
    if (blocksWithId.length === 0) {
      ...
      readableName = toolboxHelpers.getBlockAsText(
        block,
        block.parameters ? block.parameters.slice() : null,
        false
      );
    }
  ...
  launchFullEditor() {
    ...
    if (pxt.shell.isReadOnly()) {
      ...
      this.exportAsync().then((fileContent) => {
  ...
  async anonymousPublishHeaderByIdAsync(
    headerId: string,
    projectName?: string
  ): Promise<pxt.editor.ShareData> {
    const header = workspace.getHeader(headerId);
    ...
    const scrReq = {
      ...
      description: lf(
        "Made with ❤️ in {0}.",
        pxt.appTarget.title || pxt.appTarget.name
      ),
    ...
    const script: pxt.Cloud.JsonScript = await Cloud.privatePostAsync(
      "scripts",
      scrReq,
  ...
  async publishAsync(
    ...
    pxt.tickEvent("menu.embed.publish", undefined, {
      interactiveConsent: true,
    });
    ...
    try {
      ...
      const id = await this.publishCurrentHeaderAsync(
        persistentPublish,
        screenshotUri
      );
  ...
  protected async getShareUrl(pubId: string, persistent?: boolean) {
    ...
    let shareUrl =
      (persistent ? targetTheme.homeUrl : targetTheme.shareUrl) ||
      "https://makecode.com/";
    ...
    shareData.embed.code = pxt.docs.codeEmbedUrl(
      `${rootUrl}${verPrefix}`,
      pubId
    );
    shareData.embed.editor = pxt.docs.embedUrl(
      `${rootUrl}${verPrefix}`,
      "pub",
      pubId
    );
    ...
    if (pxt.webConfig.runUrl) {
      if (pxt.webConfig.isStatic) {
        ...
        simulatorRunString = pxt.webConfig.runUrl.replace(
          pxt.webConfig.relprefix,
          "/---"
        );
      }
    }
    if (pxt.appTarget.simulator)
      padding =
        (100 / pxt.appTarget.simulator.aspectRatio).toPrecision(4) + "%";
  ...
  async publishCurrentHeaderAsync(
    persistent: boolean,
    screenshotUri?: string
  ): Promise<string> {
    ...
    try {
      ...
      if (persistent) {
        const header = await workspace.persistentPublishAsync(
          epkg.header,
          files,
          meta,
          screenshotUri
        ...
        const info = await workspace.anonymousPublishAsync(
          epkg.header,
          files,
          meta,
          screenshotUri
  ...
  async saveLocalProjectsToCloudAsync(
    headerIds: string[]
  ): Promise<pxt.Map<string> | undefined> {
  ...
  updateHeaderNameAsync(name: string): Promise<void> {
    // nothing to do?
    if (pkg.mainPkg.config.name == name) return Promise.resolve();
    ...
    config.name = name;
    return f.setContentAsync(pxt.Package.stringifyConfig(config)).then(() => {
      if (this.state.header)
        this.setState({
          projectName: name,
        });
  ...
  showReportAbuse() {
    const pubId =
      (this.state.tutorialOptions &&
        this.state.tutorialOptions.tutorialReportId) ||
      (this.state.header &&
        this.state.header.pubCurrent &&
        this.state.header.pubId);
  ...
  showImportUrlDialog() {
    dialogs.showImportUrlDialogAsync().then(
      (id) => {
        if (id) {
          if (pxt.github.isGithubId(id)) importGithubProject(id);
          else loadHeaderBySharedId(id);
        }
      },
      (e) => {
  ...
  showImportGithubDialog() {
    dialogs.showImportGithubDialogAsync().then(
      (url) => {
        if (url === "NEW") {
          dialogs.showCreateGithubRepoDialogAsync().then((url) => {
            if (url) {
  ...
  showExitAndSaveDialog() {
    ...
    if (this.isTutorial()) {
      pxt.tickEvent("tutorial.exit.home", {
        tutorial: this.state.header?.tutorial?.tutorial,
      });
  ...
  showChooseHwDialog(skipDownload?: boolean) {
    if (this.chooseHwDialog) this.chooseHwDialog.show(skipDownload);
  }
  ...
  private async loadActivityFromMarkdownAsync(
    path: string,
    title?: string,
    ...
    try {
      if (/^\//.test(path)) {
        ...
        const rawMarkdown = await pxt.Cloud.markdownAsync(
          path,
          undefined,
          undefined,
          true
        ...
        if (gh?.files[pxt.TUTORIAL_INFO_FILE]) {
          await pxt.tutorial.parseCachedTutorialInfo(
            gh.files[pxt.TUTORIAL_INFO_FILE],
            path
          );
        }
        if (gh) {
    ...
    function processMarkdown(md: string) {
      ...
      if (editorProjectName == pxt.JAVASCRIPT_PROJECT_NAME) {
        // spy => typescript
        md = md.replace(/^```(blocks|block|spy)\b/gm, "```typescript");
      } else if (editorProjectName == pxt.PYTHON_PROJECT_NAME) {
        // typescript => spy
        md = md.replace(/^```(blocks|block|typescript)\b/gm, "```spy");
      }

      pxt.Util.jsonMergeFrom(
        dependencies,
        pxt.gallery.parsePackagesFromMarkdown(md)
      );
    ...
    function resolveMarkdown(
      ...
      if (pxtJson.files.find((f) => /\.ts$/.test(f))) {
        dependencies = {};
        dependencies[ghid.project] = pxt.github.toGithubDependencyPath(
          ghid.slug,
          ghid.tag
        );
      } else {
        // just use dependencies from the tutorial
        pxt.Util.jsonMergeFrom(dependencies, pxtJson.dependencies);
      }
      filename = pxtJson.name || lf("Untitled");
      autoChooseBoard = false;
  ...
  private async startTutorialAsync(
    tutorialId: string,
    ...
    if (this.state.header?.tutorial?.tutorial) {
      pxt.tickEvent("recipe.exit", {
        tutorial: this.state.header?.tutorial?.tutorial,
        goto: tutorialId,
      });
      pxt.tickEvent("tutorial.finish", {
        tutorial: this.state.header?.tutorial?.tutorial,
      });
    }
    ...
    try {
      const r = await this.loadActivityFromMarkdownAsync(
        tutorialId,
        tutorialTitle,
        editorProjectName
      );
      ...
      if (!md) throw new Error(lf("Tutorial {0} not found", tutorialId));

      const { options, editor: parsedEditor } = pxt.tutorial.getTutorialOptions(
        md,
        tutorialId,
        filename,
        reportId,
  ...
  async startActivity(opts: pxt.editor.StartActivityOptions) {
    ...
    switch (activity) {
      case "tutorial":
        ...
        await this.startTutorialAsync(
          path,
          title,
          false,
          editor,
          opts.previousProjectHeaderId,
          opts.carryoverPreviousCode
        );
        break;
      case "recipe":
  ...
  async completeTutorialAsync(): Promise<void> {
    ...
    if (tutorial && !this.state.header.isSkillmapProject) {
      ...
      if (
        this.state.tutorialOptions &&
        this.state.tutorialOptions.tutorialRecipe
      )
    ...
    if (
      ...
      this.showShareDialog(
        lf("Well done! Would you like to share your project?")
      );
    }
  ...
  exitTutorial(removeProject?: boolean) {
    ...
    this.exitTutorialAsync(removeProject).finally(() => {
      core.hideLoading("leavingtutorial");
      this.openHome();
    });
  }
    ...
    if (this.isTutorial()) {
      if (!pxt.BrowserUtils.useOldTutorialLayout()) {
        const tutorialElements =
          document?.getElementsByClassName("tutorialWrapper");
        const tutorialEl =
          tutorialElements?.length === 1
            ? (tutorialElements[0] as HTMLElement)
            : undefined;
        if (
          tutorialEl &&
          (pxt.BrowserUtils.isTabletSize() ||
            pxt.appTarget.appTheme.tutorialSimSidebarLayout)
        ) {
  ...
  pokeUserActivity() {
    if (!!this.state.tutorialOptions && !!this.state.tutorialOptions.tutorial) {
      ...
      this.hintManager.pokeUserActivity(
        ProjectView.tutorialCardId,
        this.state.tutorialOptions.tutorialStep,
        this.state.tutorialOptions.tutorialHintCounter
      );
    }
  }
  ...
  private tutorialCardHintCallback() {
    let tutorialOptions = this.state.tutorialOptions;
    tutorialOptions.tutorialHintCounter =
      tutorialOptions.tutorialHintCounter + 1;
  ...
  toggleHighContrast() {
    ...
    pxt.tickEvent("app.highcontrast", {
      on: core.getHighContrastOnce() ? 1 : 0,
    });
    if (this.isSimulatorRunning()) {
      // if running, send updated high contrast state.
      this.startSimulator();
    }
  }
  ...
  async showOnboarding() {
    const tourSteps: pxt.tour.BubbleStep[] = await parseTourStepsAsync(
      pxt.appTarget.appTheme?.tours?.editor
    );
  ...
  private handleScriptManagerDialogRef = (
    c: scriptmanager.ScriptManagerDialog
  ) => {
  ...
  renderCore() {
    ...
    const isBlocks =
      ...
      this.getPreferredEditor() === pxt.BLOCKS_PROJECT_NAME;
    const sideDocs = !(sandbox || targetTheme.hideSideDocs);
    const tutorialCompleted = !!this.state.header?.tutorialCompleted;
    const tutorialOptions = tutorialCompleted
      ? undefined
      : this.state.tutorialOptions;
    const inTutorial = !!tutorialOptions && !!tutorialOptions.tutorial;
    const isSidebarTutorial = pxt.appTarget.appTheme.sidebarTutorial;
    const isTabTutorial =
      inTutorial && !pxt.BrowserUtils.useOldTutorialLayout();
    const inTutorialExpanded =
      inTutorial && tutorialOptions.tutorialStepExpanded;
    const tutorialSimSidebar =
      pxt.appTarget.appTheme.tutorialSimSidebarLayout &&
      !pxt.BrowserUtils.isTabletSize();
    ...
    const hideTutorialIteration =
      inTutorial && tutorialOptions.metadata?.hideIteration;
    ...
    const flyoutOnly =
      this.state.editorState?.hasCategories === false ||
      (inTutorial && (tutorialOptions.metadata?.flyoutOnly || hideToolbox));
    const { hideEditorToolbar, transparentEditorToolbar } = targetTheme;
    const hideMenuBar =
      targetTheme.hideMenuBar ||
      hideTutorialIteration ||
      (isTabTutorial && pxt.appTarget.appTheme.embeddedTutorial) ||
    ...
    const showEditorToolbar =
      inEditor && !hideEditorToolbar && this.editor.hasEditorToolbar();
    const useSerialEditor =
      pxt.appTarget.serial && !!pxt.appTarget.serial.useEditor;
    const showSideDoc =
      sideDocs && this.state.sideDocsLoadUrl && !this.state.sideDocsCollapsed;
    const showCollapseButton =
      showEditorToolbar &&
      !inHome &&
      !sandbox &&
      !targetTheme.simCollapseInMenu &&
      (!isHeadless || inDebugMode) &&
      !isTabTutorial;
    const shouldHideEditorFloats =
      this.state.hideEditorFloats || this.state.collapseEditorTools;
    const logoWide = !!targetTheme.logoWide;
    const hwDialog = !sandbox && pxt.hasHwVariants();
    const editorOffset =
      (inTutorialExpanded || isTabTutorial) && this.state.editorOffset
        ? { top: this.state.editorOffset }
        : undefined;
    const invertedTheme =
      targetTheme.invertedMenu && targetTheme.invertedMonaco;
    const isMultiplayerSupported = targetTheme.multiplayer;
    const isMultiplayerGame = this.state.isMultiplayerGame;
    const collapseIconTooltip = this.state.collapseEditorTools
      ? lf("Show the simulator")
      : lf("Hide the simulator");
    ...
    const showRightChevron =
      (this.state.collapseEditorTools || isRTL) &&
      !(this.state.collapseEditorTools && isRTL); // Collapsed XOR RTL
    ...
    const showFileList =
      ...
      !(
        ...
        (pkg.mainPkg &&
          pkg.mainPkg.config &&
          pkg.mainPkg.config.preferredEditor == pxt.BLOCKS_PROJECT_NAME)
      );
    ...
    return (
      <div id="root" className={rootClasses}>
        {this.state.extensionsVisible && (
          <extensionsBrowser.ExtensionsBrowser
            ...
            importExtensionCallback={() =>
              this.showImportFileDialog({ extension: true })
            }
        ...
        {isSidebarTutorial && flyoutOnly && inTutorial && (
          <sidebarTutorial.SidebarTutorialCard
            ref={ProjectView.tutorialCardId}
            parent={this}
            pokeUser={
              this.state.pokeUserComponent == ProjectView.tutorialCardId
            }
          />
        )}
        {inTutorial && !isTabTutorial && (
          <div id="maineditor" className={sandbox ? "sandbox" : ""} role="main">
            ...
            {flyoutOnly && (
              <tutorial.WorkspaceHeader
                parent={this}
                workspaceId={this.editor.getId()}
              />
            )}
          </div>
        )}
        <sidepanel.Sidepanel
          ...
          showHostMultiplayerGameButton={
            isMultiplayerSupported && isMultiplayerGame
          }
        ...
        <div
          id="maineditor"
          className={
            (sandbox ? "sandbox" : "") + (inDebugMode ? "debugging" : "")
          }
          role="main"
          aria-hidden={inHome}
        >
          {showCollapseButton && (
            <sui.Button
              id="computertogglesim"
              className={`computer only collapse-button large`}
              icon={`inverted chevron ${showRightChevron ? "right" : "left"}`}
              title={collapseIconTooltip}
              onClick={this.toggleSimulatorCollapse}
            />
          )}
          {this.allEditors.map((e) => e.displayOuter(editorOffset))}
        </div>
        {inHome ? (
          <div id="homescreen" className="full-abs">
            <div className="ui home projectsdialog">
              <header className="menubar" role="banner">
                <accessibility.HomeAccessibilityMenu
                  parent={this}
                  highContrast={hc}
                />
                <headerbar.HeaderBar parent={this} />
        ...
        {sandbox ? undefined : (
          <scriptsearch.ScriptSearch
            parent={this}
            ref={this.handleScriptSearchRef}
          />
        )}
        {sandbox ? undefined : (
          <extensions.Extensions parent={this} ref={this.handleExtensionRef} />
        )}
        {inHome ? (
          <projects.ImportDialog
            parent={this}
            ref={this.handleImportDialogRef}
          />
        ) : undefined}
        {hasIdentity ? (
          <identity.LoginDialog parent={this} ref={this.handleLoginDialogRef} />
        ) : undefined}
        {hasIdentity ? (
          <user.ProfileDialog parent={this} ref={this.handleProfileDialogRef} />
        ) : undefined}
        {inHome && targetTheme.scriptManager ? (
          <scriptmanager.ScriptManagerDialog
            parent={this}
            ref={this.handleScriptManagerDialogRef}
            onClose={this.handleScriptManagerDialogClose}
          />
        ) : undefined}
        {sandbox ? undefined : (
          <projects.ExitAndSaveDialog
            parent={this}
            ref={this.handleExitAndSaveDialogRef}
          />
        )}
        {sandbox ? undefined : (
          <projects.NewProjectDialog
            parent={this}
            ref={this.handleNewProjectDialogRef}
          />
        )}
        {hwDialog ? (
          <projects.ChooseHwDialog
            parent={this}
            ref={this.handleChooseHwDialogRef}
          />
        ) : undefined}
        {sandbox || !sharingEnabled ? undefined : (
          <share.ShareEditor
            parent={this}
            ref={this.handleShareEditorRef}
        ...
        {this.state.onboarding && (
          <Tour
            tourSteps={this.state.onboarding}
            onClose={this.hideOnboarding}
          />
        )}
      </div>
    );
...
function parseLocalToken() {
  const qs = Util.parseQueryString(
    (location.hash || "#").slice(1).replace(/%local_token/, "local_token")
  );
  if (qs["local_token"]) {
  ...
  window.addEventListener(
    ...
    (ev: MessageEvent) => {
      ...
      if (
        ...
        pxt.packetio.sendCustomEventAsync(msg.channel, msg.data).then(
          () => {},
          (err) => {
...
function initSerial() {
  const isValidLocalhostSerial =
    pxt.appTarget.serial &&
    pxt.BrowserUtils.isLocalHost() &&
    !!Cloud.localToken;

  if (!isValidLocalhostSerial && !pxt.usb.isEnabled) return;

  if (hidbridge.shouldUse() || pxt.usb.isEnabled) return;

  pxt.debug("initializing serial pipe");
  let ws = new WebSocket(
    `ws://localhost:${pxt.options.wsPort}/${Cloud.localToken}/serial`
  );
...
function handleHash(
  ...
  switch (newHash.cmd) {
    ...
    case "newpython": // shortcut to create a new python proj
      ...
      return true;
    case "testproject": {
      // create new project that references the given extension
      ...
      if (header) {
        const existing = workspace
          .getHeaders()
          .filter((hd) => hd.extensionUnderTest == header.id)[0];
        if (existing) editor.loadHeaderAsync(existing);
        else {
          editor.newProject({
    ...
    case "recipe": {
      ...
      if (/^([jt]s|py|blocks?):/i.test(tutorialPath)) {
        ...
        else editorProjectName = pxt.BLOCKS_PROJECT_NAME;
    ...
    case "edit": // load a published proj, eg: #pub:27750-32291-62442-22749
      ...
      else loadHeaderBySharedId(newHash.arg);
      return true;
    case "header":
      pxt.tickEvent("hash." + newHash.cmd);
      pxt.BrowserUtils.changeHash("");
    ...
    case "project":
      ...
      editor
        .importProjectFromFileAsync(fileContents)
    ...
    case "skillmapimport":
      ...
      editor.importSkillmapProjectAsync(headerId).finally(() => {
        pxt.BrowserUtils.changeHash("");
    ...
    case "embedimport":
      ...
      editor.importEmbedProjectAsync(importId).finally(() => {
        pxt.BrowserUtils.changeHash("");
    ...
    case "github": {
      ...
      if (ghCmd === "create-repository") {
        ...
        if (hd) {
          ...
          if (!provider.hasToken()) return false;
          theEditor
            .loadHeaderAsync(hd)
...
async function importGithubProject(repoid: string, requireSignin?: boolean) {
  if (
    !pxt.appTarget.appTheme.githubEditor ||
    pxt.BrowserUtils.isPxtElectron()
  ) {
    core.warningNotification(
      lf("Importing GitHub projects not currently supported")
    );
    theEditor.openHome();
    return;
  }
  ...
  try {
    ...
    let hd = workspace
      .getHeaders()
      .find(
        (h) => h.githubId && pxt.github.normalizeRepoId(h.githubId) == repoid
      );
    if (!hd) {
      if (requireSignin) {
        const token = await cloudsync
          .githubProvider(true)
          .routedLoginAsync(repoid);
        if (!token.accessToken) {
          // did not sign in, give up
          theEditor.openHome();
          return;
        }
      }
    ...
    if (hd) await theEditor.loadHeaderAsync(hd, null);
    else theEditor.openHome();
  } catch (e) {
    core.handleNetworkError(e);
    theEditor.openHome();
  } finally {
...
function saveAsBlocks(): boolean {
  try {
    return (
      /save(?:as)?blocks=1/i.test(window.location.href) &&
      !!pkg.mainPkg.readFile(pxt.MAIN_BLOCKS)
    );
  } catch (e) {
    return false;
  }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (optsQuery["wsport"]) {
    ...
    pxt.BrowserUtils.changeHash(
      window.location.hash.replace(`wsport=${optsQuery["wsport"]}`, "")
    );
  } else {
    pxt.options.wsPort = 3233;
  }
  if (
    optsQuery["consoleticks"] == "1" ||
    optsQuery["consoleticks"] == "verbose"
  ) {
    pxt.analytics.consoleTicks = pxt.analytics.ConsoleTickOptions.Verbose;
  } else if (
    optsQuery["consoleticks"] == "2" ||
    optsQuery["consoleticks"] == "short"
  ) {
  ...
  const isControllerIFrame =
    theme.allowParentController || pxt.shell.isControllerMode();
  // disable auth in iframe scenarios
  if (isControllerIFrame) pxt.auth.enableAuth(false);
  enableAnalytics();

  if (
    !pxt.BrowserUtils.isBrowserSupported() &&
    ...
    core.showLoading(
      "browsernotsupported",
      lf("Sorry, this browser is not supported.")
    );
    return;
    ...
    core.showLoading(
      "micro:bit-winrt-reload",
      lf("Redirecting to MakeCode micro:bit v4")
    );
    return;
  }
  ...
  if (pxt.appTarget.cloud?.apiRoot) Cloud.apiRoot = pxt.appTarget.cloud.apiRoot;
  else {
  ...
  Promise.resolve()
    .then(async () => {
      ...
      auth.initialUserPreferencesAsync().then((pref) => {
        ...
        if (
          ...
          pref &&
          pref.language &&
          pref.language != cookieLang
        ) {
          pxt.BrowserUtils.setCookieLang(pref.language);
          core.infoNotification(
            lf("Reload the page to apply your new language preference.")
          );
      ...
      if (/[&?]translate=1/.test(href) && !pxt.BrowserUtils.isIE()) {
        ...
        const hashLangMatch = /(live)?(force)?lang=([a-z]{2,}(-[A-Z]+)?)/i.exec(
          window.location.href
        );
        if (
          hashLangMatch &&
          window.location.hash.indexOf(hashLangMatch[0]) >= 0
        ) {
          pxt.BrowserUtils.changeHash(
            window.location.hash.replace(hashLangMatch[0], "")
          );
        }
        ...
        const stringUpdateDisabled =
          locstatic || serveLocal || theme.disableLiveTranslations;
      ...
      return Util.updateLocalizationAsync({
        ...
        if (pxt.Util.isLocaleEnabled(useLang)) {
          ...
          pxt.tickEvent("unavailablelocale", {
            lang: useLang,
            force: force ? "true" : "false",
          });
        }
        pxt.tickEvent("locale", {
          lang: pxt.Util.userLanguage(),
          live: pxt.Util.liveLocalizationEnabled() ? "true" : "false",
        });
        ...
        Util.downloadTranslationsAsync(
          ...
          Util.TranslationsKind.Sim
        ).then(
          (simStrings) => simStrings && simulator.setTranslations(simStrings)
        );
      });
    })
    .then(() => {
    ...
    .then((state) => {
      ...
      if (state) theEditor.setState({ editorState: state });
    ...
    .then(() => {
      ...
      // default handlers
      const ent = theEditor.settings.fileHistory.filter(
        (e) => !!workspace.getHeader(e.id)
      )[0];
  ...
  document.addEventListener("visibilitychange", async (ev) => {
    if (theEditor) await theEditor.updateVisibilityAsync();
  });

  window.addEventListener("unload", (ev) => {
    if (theEditor) theEditor.saveSettings();
  ...
  window.addEventListener(
    ...
    (ev) => {
      ...
      if (ev.data.__proxy == "parent") {
        ...
        const ipcRenderer = (window as any).ipcRenderer;
        if (ipcRenderer) ipcRenderer.sendToHost("sendToApp", ev.data);
      ...
      if (m.type == "tutorial" || m.type == "opendoc") {
        if (theEditor && theEditor.editor) theEditor.handleMessage(m);
        return;
      }
      if (
        m.type === "sidedocready" &&
      ...
      if (m.type == "importfile") {
        ...
        if (theEditor) theEditor.importFile(new File(msg.parts, msg.filename));
        return;
      }
    },
    false
