...
export class EditorToolbar extends data.Component<
  ISettingsProps,
  EditorToolbarState
> {
  ...
  startStopSimulator(view?: string) {
    pxt.tickEvent(
      "editortools.startStopSimulator",
      {
        ...
        headless: this.getHeadlessState(),
      },
      { interactiveConsent: true }
    );
  ...
  toggleCollapsed() {
    pxt.tickEvent(
      "editortools.portraitToggleCollapse",
      {
        collapsed: this.getCollapsedState(),
        headless: this.getHeadlessState(),
      },
    ...
    if (this.props.parent.state.compiling) {
      ...
      if (this.props.parent.state.cancelledDownload) {
        ...
        this.compileTimeout = setTimeout(() => {
          if (this.state?.compileState === "success")
            this.setState({ compileState: null });
        }, 2000);
      }
    }
  ...
  private getSaveInput(
    ...
    let saveInput = [];
    saveInput.push(
      <label htmlFor={id} className="accessible-hidden phone hide" key="label">
    ...
    saveInput.push(
      <EditorToolbarSaveInput
        id={id}
        view={this.getViewString(View.Computer)}
        key="input"
        type="text"
        aria-labelledby={id}
    ...
    if (showSave) {
      saveInput.push(
        <EditorToolbarButton
          role="button"
          icon="save"
          className={`right attached editortools-btn save-editortools-btn ${saveButtonClasses}`}
          title={lf("Save")}
          ariaLabel={lf("Save the project")}
          onButtonClick={this.saveFile}
          view={this.getViewString(View.Computer)}
          key={`save${View.Computer}`}
        />
      );
    }
  ...
  protected getUndoRedo(view: View): JSX.Element[] {
    ...
    return [
      <EditorToolbarButton
        icon="xicon undo"
        className={`editortools-btn undo-editortools-btn ${
          !hasUndo ? "disabled" : ""
        }`}
        title={lf("Undo")}
      ...
      <EditorToolbarButton
        icon="xicon redo"
        className={`editortools-btn redo-editortools-btn ${
          !hasRedo ? "disabled" : ""
        }`}
        title={lf("Redo")}
  ...
  protected onHwItemClick = () => {
    if (pxt.hasHwVariants()) this.props.parent.showChooseHwDialog(true);
    else this.props.parent.showBoardDialogAsync(undefined, true);
  };

  protected onDownloadButtonClick = async () => {
    pxt.tickEvent(
      "editortools.downloadbutton",
      { collapsed: this.getCollapsedState() },
      { interactiveConsent: true }
    );
    if (
      this.shouldShowPairingDialogOnDownload() &&
  ...
  protected onCannotPairClick = async () => {
    pxt.tickEvent("editortools.pairunsupported", undefined, {
      interactiveConsent: true,
    });
    ...
    switch (reasonUnsupported) {
      case "security":
        modalBody = lf(
          "WebUSB is disabled by browser policies. Check with your admin for help."
        );
        break;
      case "oldwindows":
        modalBody = lf(
          "WebUSB is not available on Windows devices with versions below 8.1."
        );
        break;
      case "electron":
      ...
      case "notimpl":
        modalBody = lf(
          "WebUSB is not supported by this browser; please check for updates."
        );
        break;
    }
  ...
  protected onHelpClick = () => {
    ...
    window.open(
      pxt.appTarget.appTheme.downloadDialogTheme?.downloadMenuHelpURL
    );
  };
  ...
  protected getCompileButton(view: View): JSX.Element[] {
    ...
    const compileTooltip = lf(
      "Download your code to the {0}",
      targetTheme.boardName
    );

    let downloadText: string;
    if (compileState === "success") {
      downloadText = targetTheme.useUploadMessage
        ? lf("Uploaded!")
        : lf("Downloaded!");
    } else {
      downloadText = targetTheme.useUploadMessage
    ...
    const showUsbNotSupportedHint =
      editorSupportsWebUSB &&
      !pxt.usb.isEnabled &&
    ...
    const hideFileDownloadIcon =
      view === View.Computer && this.shouldShowPairingDialogOnDownload();
    const fileDownloadIcon = targetTheme.downloadIcon || "xicon file-download";

    const successIcon =
      (packetioConnected &&
        pxt.appTarget.appTheme.downloadDialogTheme?.deviceSuccessIcon) ||
      "xicon file-download-check";
    const downloadIcon =
      (!!packetioConnecting && "ping " + packetioIcon) ||
    ...
    if (packetioConnected) downloadButtonClasses += "connected ";
    else if (packetioConnecting) downloadButtonClasses += "connecting ";
    switch (view) {
      case View.Mobile:
    ...
    el.push(
      <EditorToolbarButton
        ...
        onButtonClick={this.onDownloadButtonClick}
        view="computer"
      />
    );

    const deviceName =
      pxt.hwName || pxt.appTarget.appTheme.boardNickname || lf("device");
    const tooltip =
      pxt.hwName ||
      (packetioConnected && lf("Connected to {0}", deviceName)) ||
      (packetioConnecting && lf("Connecting...")) ||
      (boards
        ? lf("Click to select hardware")
        : webUSBSupported
        ? lf("Click for one-click downloads.")
        : undefined);

    const hardwareMenuText =
      view == View.Mobile ? lf("Hardware") : lf("Choose Hardware");
    const downloadMenuText =
      view == View.Mobile
        ? pxt.hwName || lf("Download")
        : lf("Download as File");
    const downloadHelp =
      pxt.appTarget.appTheme.downloadDialogTheme?.downloadMenuHelpURL;
    ...
    // Add the ... menu
    const usbIcon =
      pxt.appTarget.appTheme.downloadDialogTheme?.deviceIcon || "usb";
    el.push(
      <sui.DropdownMenu
        key="downloadmenu"
        role="menuitem"
        icon={`${downloadButtonIcon} horizontal ${hwIconClasses}`}
        title={lf("Download options")}
        className={`${hwIconClasses} right attached editortools-btn hw-button button`}
        dataTooltip={tooltip}
        displayAbove={true}
        displayRight={displayRight}
      >
        {webUSBSupported && !packetioConnected && (
          <sui.Item
            role="menuitem"
            icon={usbIcon}
            text={lf("Connect Device")}
            tabIndex={-1}
            onClick={this.onPairClick}
          />
        )}
        {showUsbNotSupportedHint && (
          <sui.Item
            role="menuitem"
            icon={usbIcon}
            text={lf("Connect Device")}
            tabIndex={-1}
            onClick={this.onCannotPairClick}
          />
        )}
        {webUSBSupported && (packetioConnecting || packetioConnected) && (
          <sui.Item
            role="menuitem"
            icon={usbIcon}
            text={lf("Disconnect")}
        ...
        {boards && (
          <sui.Item
            role="menuitem"
            icon="microchip"
            text={hardwareMenuText}
            tabIndex={-1}
            onClick={this.onHwItemClick}
          />
        )}
        <sui.Item
          role="menuitem"
          icon="xicon file-download"
          text={downloadMenuText}
          tabIndex={-1}
          onClick={this.onHwDownloadClick}
        />
        {downloadHelp && (
          <sui.Item
            role="menuitem"
  ...
  renderCore() {
    ...
    const header =
      this.getData(`header:${this.props.parent.state.header.id}`) ??
      this.props.parent.state.header;
    ...
    const disableFileAccessinMaciOs =
      targetTheme.disableFileAccessinMaciOs &&
      (pxt.BrowserUtils.isIOS() || pxt.BrowserUtils.isMac());
    const disableFileAccessinAndroid =
      pxt.appTarget.appTheme.disableFileAccessinAndroid &&
    ...
    const showSave =
      !readOnly &&
      !isController &&
      !targetTheme.saveInMenu &&
      !tutorial &&
      !debugging &&
      !disableFileAccessinMaciOs &&
      !disableFileAccessinAndroid &&
      !hasRepository;
    const showProjectRename =
      !tutorial &&
      !readOnly &&
      !isController &&
      !targetTheme.hideProjectRename &&
      !debugging;
    ...
    return (
      ...
      >
            ...
            <div className="ui item">
              <div className="ui icon large buttons">
                {compileBtn && (
                  <EditorToolbarButton
                    icon={downloadIcon}
                    className={`primary large download-button mobile tablet hide ${downloadButtonClasses}`}
                    title={compileTooltip}
        ...
        {(showProjectRename ||
          showGithub ||
          identity.CloudSaveStatus.wouldRender(header.id)) && (
          <div id="projectNameArea" className="ui column items">
            <div
              className={`ui right ${
                showSave ? "labeled" : ""
              } input projectname-input projectname-computer`}
            >
              {showProjectRename &&
                this.getSaveInput(
                  showSave,
                  "fileNameInput2",
                  projectName,
                  showProjectRenameReadonly
                )}
              {showGithub && (
                <githubbutton.GithubButton
                  parent={this.props.parent}
                  key={`githubbtn${computer}`}
                />
              )}
        ...
        <div id="editorToolbarArea" role="menubar" className="ui column items">
          {showUndoRedo && (
            <div className="ui icon buttons">{this.getUndoRedo(computer)}</div>
          )}
          {showZoomControls && (
            <div className="ui icon buttons mobile hide">
          ...
          {targetTheme.bigRunButton && !pxt.shell.isTimeMachineEmbed() && (
            <div className="big-play-button-wrapper">
              <EditorToolbarButton
                className={`big-play-button play-button ${
                  running ? "stop" : "play"
                }`}
                key="runmenubtn"
                disabled={starting}
...
export class ZoomSlider extends data.Component<
  ZoomSliderProps,
  ZoomSliderState
> {
  ...
  constructor(props: ZoomSliderProps) {
    ...
    this.state = {
      zoomValue:
        Math.floor((this.zoomMax + 1 - this.zoomMin) / 2) + this.zoomMin,
    };
  ...
  renderCore() {
    return (
      <div className="zoom">
        ...
        <div id="zoomSlider">
          <input
            className="zoomSliderBar"
            type="range"
            min={this.zoomMin}
            max={this.zoomMax}
            step="1"
            value={this.state.zoomValue.toString()}
            onChange={this.zoomUpdate}
...
export class SmallEditorToolbar extends EditorToolbar {
  ...
  renderCore() {
    return (
      <div id="headerToolbar" className="smallEditorToolbar">
        <ZoomSlider
          parent={this.props.parent}
          view={super.getViewString(View.Computer)}
          zoomMin={0}
          zoomMax={5}
        ></ZoomSlider>
        <div className="ui icon undo-redo-buttons">
          {super.getUndoRedo(View.Computer)}
        </div>
      </div>
    );
  }
...
class EditorToolbarSaveInput extends React.Component<
  EditorToolbarSaveInputProps,
  EditorToolbarSaveInputState
> {
